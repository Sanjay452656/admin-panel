...
import { LayoutDashboard, Server, Package, ListTree, ShoppingCart, BarChart3, Users, Settings, Building2 } from 'lucide-react';
  ...
  return (
    <aside className="w-64 bg-gray-900 text-white min-h-screen flex flex-col shrink-0">
      <div className="p-4 border-b border-gray-800">
        <div className="text-2xl font-bold">M9Vends</div>
        {user.role === 'SUPER_ADMIN' && (
          <div className="mt-1 text-xs font-medium text-indigo-400 uppercase tracking-wider">Platform Admin</div>
        )}
      </div>
      ...
      <div className="p-4 border-t border-gray-800 text-sm text-gray-400">
        ...
        <div className="capitalize text-xs mt-0.5">{user.role.replace(/_/g, ' ').toLowerCase()}</div>
      </div>
    </aside>
  );
}
